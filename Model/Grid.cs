    ...
    public abstract class Grid
    {
        ...
        public IEnumerable<SquareSequence> GetAvailablePlacements(int length)
        {
            return GetPlacements(length, new LoopIndex(Rows, Columns), (i, j) => squares[i, j])
                .Concat(GetPlacements(length, new LoopIndex(Columns, Rows), (i, j) => squares[j, i])).Where(pl => pl.Any());
        }
        ...
        protected abstract bool IsSquareAvailable(int row, int column, Func<int, int, Square> squareSelect);
